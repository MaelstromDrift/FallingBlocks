...
public class TutorialWorld {
	...
	public void updateChar(float deltaTime, float accelX) {
		...
		if (character.position.y > 222 && character.position.y < 242) {
			if (character.position.x - 16 > blocks.get(1).position.x - 50
					&& character.position.x + 16 < blocks.get(1).position.x + 50) {
				blocks.get(2).state = Block.STATE_FALLING;
			}
		}
		if (score < character.position.y) {
			score = (int) character.position.y;
		}
	}
		...
		for (int i = 0; i < blocks.size(); i++) {
			if (OverlapTester.overlapRectangles(character.bounds,
				...
				if (character.position.y - 25 < blocks.get(i).position.y
						...
						&& character.position.y + 25 > blocks.get(i).position.y
								- blocks.get(i).width / 2) {
					if (character.position.x + 25 < blocks.get(i).position.x) {
	...
	public void checkBoxCollision(float deltaTime) {
		for (int j = 0; j < blocks.size(); j++) {
			for (int i = 0; i < blocks.size(); i++) {
				...
				if (ContinuousCollision.checkYCollision(
						blocks.get(j).position.y, 50, blocks.get(j).velocity.y
								* deltaTime, 0, character.position.y, 25)) {
					if (blocks.get(j).position.x - (blocks.get(j).width / 2)
							- 4 < character.position.x + 16
							&& blocks.get(j).position.x
									+ (blocks.get(j).width / 2) - 4 > character.position.x - 16) {
